...
function VolumeDetailPanelBody({
    ...
    const [shareableRoots, setShareableRoots] = useState<deploy.RootServiceSummary[]>([]);
    const [cloneSourceId, setCloneSourceId] = useState('');
    const [cloneConsistency, setCloneConsistency] = useState<'consistent' | 'quick'>('consistent');
    ...
    return (
        <div className="node-detail-panel">
            ...
            <div className="node-detail-body">
                ...
                {isNamed && (
                    <div className="settings-section">
                        <h3 className="settings-section-title">Clone contents from…</h3>
                        <span className="settings-hint">
                            Replace this volume&apos;s data with a copy from another environment&apos;s root service.
                            The previous volume is kept as an orphan until you delete it. Target is always stopped first.
                        </span>
                        ...
                        <div className="form-field">
                            ...
                            <select
                                className="input settings-select"
                                value={cloneConsistency}
                                onChange={(e) => setCloneConsistency(e.target.value as 'consistent' | 'quick')}
                            >
                        ...
                        <button
                            ...
                            disabled={!cloneSourceId || cloneBusy}
                            onClick={async () => {
                                if (!cloneSourceId || !entry.containerPath) return;
                                setCloneBusy(true);
                                try {
                                    const preview = await PreviewCloneVolume(parentNodeId, cloneSourceId, entry.containerPath);
                                    if (!await confirm({
                                        title: 'Replace volume data?',
                                        message: `Replace all data at ${entry.containerPath} with a copy from the selected service.`,
                                        detail: preview?.warning || 'Previous volume will be kept as an orphan until you delete it.',
                                    ...
                                    const result = await CloneVolumeData(parentNodeId, cloneSourceId, entry.containerPath, cloneConsistency);
                                    await alert({
                                        title: 'Volume cloned',
                                        message: result?.orphanedVolumeName
                                            ? `New volume: ${result.newVolumeName}. Previous volume kept as orphan: ${result.orphanedVolumeName}. Deploy to use the new data.`
                                            : `New volume: ${result?.newVolumeName || 'ready'}. Deploy to use the new data.`,
                                    });
                                    refreshManagedVolumes();
                                    onVolumesChanged?.();
                                } catch (e) {
                                    await alert({title: 'Clone failed', message: String(e)});
                                } finally {
                                    setCloneBusy(false);
                                }
                            }}
                        >
                            {cloneBusy ? 'Cloning…' : 'Clone from source'}
                        </button>
                    </div>
                )}
            </div>
